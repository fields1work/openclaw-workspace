...
print(f"🎙️ Generating {len(scripts)} FREE voiceovers with Windows TTS...\n")

for i, (filename, text) in enumerate(scripts, 1):
    output_file = os.path.join(output_dir, f"voiceover_{filename}_free.mp3")
    
    # Skip if already exists (from ElevenLabs)
    if os.path.exists(output_file.replace("_free", "")):
        print(f"[{i}/9] ⏭️  Skipping {filename} (ElevenLabs version exists)")
        continue
    
    print(f"[{i}/9] 🎙️ Generating {filename}...")
...
print(f"\n🎉 BATCH COMPLETE!")
print(f"💰 Total cost: $0.00 (Windows TTS)")
